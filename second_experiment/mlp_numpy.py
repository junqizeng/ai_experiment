...
def content_with_time(content):
    return f'{get_time()} {content}'
...
class MLP:
    ...
    def backward(self):
        ...
        g_x2_w2 = self.out1
        g_loss_x2 = g_loss_out2 * g_out2_x2
        self.gradient_dict['output_layer_weights'] = g_loss_x2 @ g_x2_w2.T  # [output, hidden]
        self.gradient_dict['output_layer_bias'] = g_loss_x2

        g_x2_out1 = self.param_dict['output_layer_weights']
        g_out1_x1 = np.where(self.out1 != 0, 1, 0)
        g_x1_w1 = self.x0
        g_loss_x1 = (g_x2_out1.T @ g_loss_x2) * g_out1_x1
        self.gradient_dict['hidden_layer_weights'] = g_loss_x1 @ g_x1_w1.T
        self.gradient_dict['hidden_layer_bias'] = g_loss_x1
        return
...
if __name__ == '__main__':
    ...
    for i in range(max_epoch):
        ...
        train_result_str = content_with_time(f"epoch {i} loss {sum(loss_list) / len(loss_list)}")
        ...
        test_result_str = content_with_time(f"epoch {i} acc {sum(acc_list)/len(acc_list)}")
